...
$(function(){
    ...
    $.post("/mboss__init__", postdata).done((res) => {
        $.post("/get_client_details", {
            ...
            if (res["notfound"]){
                message_containner.html("<h1>400 Bad Request</h1><br>unknown miniboss");
                return;
            }
            ...
            if (res["alr"]){
                message_containner.html(
                    "<h1>あなたは既にこのミニボスを撃破しています！</h1><br>他のQRコードを探してください！"
                );
                return;
            }
            if (res["neu"] && !res["new"]){
                message_containner.html(
                    `<h1>ミニボスと闘うにはあと${res["needed"]}個のクイズを終わらせないといけません！</h1><br>
                    頑張って探してきてください！`
                );
                return;
            }
            if (res["new"]){
                ...
                var _missing = "";
                for (var type of missing){
                    _missing += type + ", ";
                }
                _missing = _missing.slice(0, -2);

                message_containner.html(
                    `<h1>ミニボスと戦うには武器を 3 種類所持していなければなりません！</h1><br><h2>不足している武器: ${_missing}</h2>`
                );
                return;
            }

            BOSS_MAX_HEALTH = Math.floor(BOSS_MAX_HEALTH*(clientData["finishedevents"].length + 1)/3);
            BOSS_CURRENT_HEALTH = BOSS_MAX_HEALTH;
            MBOSS_SLAIN_COUNT = data["finishedmbosses"].length;
            MAX_PATTERN_LENGTH = MBOSS_SLAIN_COUNT +2;
            $("#boss_health_num").text(String(BOSS_CURRENT_HEALTH));
            // miniboss slainable up to 4 times
            switch (MAX_PATTERN_LENGTH){
                // slain 2 times so far
                case 4:
                    MIN_PATTERN_LENGTH = 2;
                    SECOND_OF_A_TURN = 6;
                    break;
                // slain 3 times so far (this is the last miniboss)
                case 5:
            ...
            message_containner.html(
                `<h1>ミニボスと戦いますか？<br>(レベル: ${MBOSS_SLAIN_COUNT +1})<br>下のボタンをおして開始してください
                </h1><br>
...
function showWinScreen(fromEER=false){
    ...
    $.post("/mboss__clear__", {
        ...
        setTimeout(function(){
            ...
            $(".logSession").show();
            $(".sessionMessage").html(`
                <h1 style="margin: 0; color: red;">ミニボスを倒しました！</h1><br>
                <h2 style="margin: 0;">加護がインベントリに追加されました</h2>
                <h2 style="margin: 0; color: green;">加護の役割についてはインベントリでご覧ください</h2>
            `);
            $(".startbtn__").hide();
...
function setBossHealth(amount){
    var el = document.getElementsByClassName("h_b")[0];
    BOSS_CURRENT_HEALTH -= amount;
